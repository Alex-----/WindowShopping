...
public class MarkovAllKPredictor extends Predictor {
	...
	public Boolean Train(List<Sequence> trainingSequences) {
		...
		int ko = parameters.paramIntOrDefault("order", K);
		...
		for(Sequence seq : trainingSequences) {
			...
			for(int i = 0 ; i < (items.size() - 1); i++) {
				...
				for(int c = 1 ; c <= k ; c++) {
					...
					MarkovState state = mDictionary.computeIfAbsent(key, kk -> new MarkovState());
			...
			if(!items.isEmpty()){
				mDictionary.computeIfAbsent("", k -> new MarkovState()).addTransition(items.get(0).val);
			}
		}
	...
	public Sequence Predict(Sequence target) {
		...
		if(k == 0){
			MarkovState state = mDictionary.get("");
			if(state != null) {
